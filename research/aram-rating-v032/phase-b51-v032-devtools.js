...
(()=>{
  ...
  async function runB51(options={}){
    ...
    try{
      const C=core(),cp=assertCp(await readCp(),C);cp.completed_puuids_b51_v032=Array.isArray(cp.completed_puuids_b51_v032)?cp.completed_puuids_b51_v032:[];cp.expansion_logs_b51_v032=Array.isArray(cp.expansion_logs_b51_v032)?cp.expansion_logs_b51_v032:[];makeQueue(C,cp);
      const beforeNow=summary(C,cp,'before_b51_v032');cp.phase='B5.1';cp.sampling_version=VERSION;cp.sampling_profile=CFG.sampling_profile;cp.b51_v032_before=cp.b51_v032_before||beforeNow;cp.status='b51_v032_collecting';await writeCp(cp);
      if(beforeNow.checkpoint_matches>=CFG.max_total_accepted_matches){cp.status='hard_cap_reached';cp.b51_v032_final=beforeNow;await writeCp(cp);return{status:cp.status,expanded_this_run:0,requests_this_run:0,before:cp.b51_v032_before,after:beforeNow,privacy:{contains_raw_puuid:false,contains_riot_id:false},production_changed:false}}
      let expanded=0;
      while(true){
        ...
        const done=new Set(cp.completed_puuids_b51_v032||[]),f=(cp.b51_v032_queue||[]).find(x=>!done.has(x.puuid));if(!f){cp.status='phase_complete';break}
        ...
        if(!merged){cp.expansion_logs_b51_v032.push({anchor_rank:f.anchor_rank,source_pool:f.source_pool,evidence_tier:f.evidence_tier,heldout_matches:f.heldout_matches,closed_10_of_10_matches:f.closed_10_of_10_matches,requests:attempts,new_unique_matches:0,error:err});cp.completed_puuids_b51_v032.push(f.puuid);await writeCp(cp);continue}
        ...
        const now=summary(C,cp,'progress');console.log(`[ARAM Rating v0.3.2] B5.1 ${now.queue_completed}/${now.queue_selected} · tier ${f.evidence_tier} · +${accepted.length} · live 10/10 ${yieldStats.closed_10_of_10_matches}/${yieldStats.matches} · accepted ${now.checkpoint_matches}/${CFG.max_total_accepted_matches} · no-cold ${(100*now.no_cold_test_fraction).toFixed(2)}%`);
      ...
      const out={status:cp.status,expanded_this_run:expanded,requests_this_run:runtime.requests,before:cp.b51_v032_before,after,delta:cp.b51_v032_delta,queue:queueProgress(cp),privacy:{contains_raw_puuid:false,contains_riot_id:false},production_changed:false};console.log('[ARAM Rating v0.3.2] B5.1 stopped',out);return out;
    }finally{runtime=null;delete globalThis[LOCK]}
  }
  function abort(){if(runtime)runtime.abort=true;return!!runtime}
  async function status(){const C=core(),cp=assertCp(await readCp(),C);return{...summary(C,cp,'current'),status:cp.status||null,sampling_profile:cp.sampling_profile||null,automatic_collection:false}}
  window.aramRatingB51V032={version:VERSION,config:{...CFG},preview,runB51,abort,status,confirmation:CONFIRM};
  console.log('[ARAM Rating v0.3.2] B5.1 manual evidence-first runner ready. NO Riot/LCU collection request has started.');
})();