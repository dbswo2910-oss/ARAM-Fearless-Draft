'use strict';
/* ARAM Rating v0.3.2 B4 mature-network aggregate evaluator. Read-only: never writes checkpoint and never starts Riot/LCU collection. */
(()=>{
  const VERSION='v0.3.2',DB='aram-rating-research-v03',STORE='kv',CP_KEY='checkpoint-v03',MIN_MATCHES=1900;
  function engine(){const E=globalThis.ARAMRatingResearchEngineV01;if(!E)throw new Error('reference_engine_not_loaded_load_b2_evaluator_once');return E}
  ...
  async function evaluate(){
    ...
    const E=engine(),normalized=E.normalizeMatches(cp.matches);if(normalized.length<MIN_MATCHES)throw new Error(`b4_1900_required_current_${normalized.length}`);
    const run=E.buildLatestRun(cp.matches,{phase:'B4',status:cp.status||'phase_complete',sampling_version:VERSION,source:'local_checkpoint_v032_mature_network'});
    const summary={
      status:'SUCCESS',evaluation:'B4_MATURE_NETWORK_AGGREGATE',sampling_version:VERSION,engine_version:run.engine_version,
      checkpoint_status:cp.status||null,b4_final:cp.b4_v032_final||null,
    ...
    console.log('[ARAM Rating v0.3.2] B4 mature-network aggregate evaluation complete',summary);return summary;
  }
  window.aramRatingB4EvaluateV032={version:VERSION,evaluate};
  console.log('[ARAM Rating v0.3.2] B4 evaluator ready. READ ONLY · requires preloaded reference engine · run: await aramRatingB4EvaluateV032.evaluate()');
})();
