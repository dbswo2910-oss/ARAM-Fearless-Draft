...
(function(root,factory){
  ...
  function matchLevelScoreCandidate(c={},cfg={}){
    ...
    const sampleBonus=Math.min(2.5,n*.45),evidenceSupport=clamp(n/5),priority=.22*base.b5_priority+18*r10+8*r9+3.5*r8+6.0*known+3.5*known5+8.0*lower+9.0*directYield+sampleBonus+4.0*evidenceSupport-9.0*unknownPenalty;
    return{...base,b51_priority:priority,match_level_closed_priority:priority,direct_no_cold_yield_proxy:directYield,heldout_evidence:evidence,heldout_sample_bonus:sampleBonus,avg_unknown_penalty:unknownPenalty,evidence_tier:tier,evidence_support:evidenceSupport};
  }
  function evidenceFirstCompare(a,b){
    return b.evidence_tier-a.evidence_tier||b.closed_10_of_10_wilson_lower-a.closed_10_of_10_wilson_lower||b.closed_10_of_10_ratio-a.closed_10_of_10_ratio||b.exact_9_of_10_ratio-a.exact_9_of_10_ratio||b.exact_8_of_10_ratio-a.exact_8_of_10_ratio||b.heldout_matches-a.heldout_matches||b.direct_no_cold_yield_proxy-a.direct_no_cold_yield_proxy||b.b51_priority-a.b51_priority||String(a.puuid).localeCompare(String(b.puuid));
  }
  function buildMatchLevelCandidatePool(checkpoint,cfg={}){
    ...
    return rows.map(x=>matchLevelScoreCandidate(x,cfg)).filter(x=>!x.skip).sort(evidenceFirstCompare);
  }
  return{...B5,wilsonLower,matchKnownProfile,directMatchLevelStats,evidenceTier,evidenceFirstCompare,matchLevelScoreCandidate,buildMatchLevelCandidatePool,policy_version:'v0.3.2-b5.1-evidence-first-preview',phase:'B5.1',automatic_collection:false,live_requests:false};
});
