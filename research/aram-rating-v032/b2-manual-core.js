...
(function(root,factory){
  ...
  function components(state){const seen=new Set(),out=[],sizeByNode=new Map();for(const p of state.adj.keys()){if(seen.has(p))continue;const nodes=[],stack=[p];seen.add(p);while(stack.length){const u=stack.pop();nodes.push(u);for(const v of state.adj.get(u)||[])if(!seen.has(v)){seen.add(v);stack.push(v)}}out.push(nodes);for(const n of nodes)sizeByNode.set(n,nodes.length)}out.sort((a,b)=>b.length-a.length);return{components:out,sizeByNode}}
  ...
  function buildCandidatePool(checkpoint,cfg={}){
    ...
    for(const[puuid,obs]of cur.counts){
      ...
      const recent=Math.min(historyLimit,(cur.byPlayer.get(puuid)||[]).length),neighbors=[...(cur.adj.get(puuid)||[])],repeatNeighbors=neighbors.filter(x=>(cur.counts.get(x)||0)>=2).length,overlap=neighbors.length?repeatNeighbors/neighbors.length:0,componentSize=comp.sizeByNode.get(puuid)||1,componentRatio=componentSize/players,head=Math.max(0,historyLimit-recent);
  ...
  const n=(o,k)=>Number(o?.[k]??(k==='matches'?o?.checkpoint_matches:0))||0;
  function delta(before={},after={}){return{matches:n(after,'matches')-n(before,'matches'),players:n(after,'players')-n(before,'players'),single_match_fraction:n(after,'single_match_fraction')-n(before,'single_match_fraction'),players_2_plus:n(after,'players_2_plus')-n(before,'players_2_plus'),players_5_plus:n(after,'players_5_plus')-n(before,'players_5_plus'),players_10_plus:n(after,'players_10_plus')-n(before,'players_10_plus'),giant_component_ratio:n(after,'giant_component_ratio')-n(before,'giant_component_ratio'),connected_components:n(after,'connected_components')-n(before,'connected_components')}}
