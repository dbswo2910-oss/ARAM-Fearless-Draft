'use strict';
/* ARAM Rating v0.3.2 B3 aggregate evaluator. Read-only: never writes checkpoint and never starts Riot/LCU collection. */
(()=>{
  const VERSION='v0.3.2',DB='aram-rating-research-v03',STORE='kv',CP_KEY='checkpoint-v03',MIN_MATCHES=1000;
  ...
  async function evaluate(){
    ...
    const E=engine(),normalized=E.normalizeMatches(cp.matches);if(normalized.length<MIN_MATCHES)throw new Error(`b3_1000_required_current_${normalized.length}`);
    const run=E.buildLatestRun(cp.matches,{phase:'B3',status:cp.status||'hard_cap_reached',sampling_version:VERSION,source:'local_checkpoint_v032'});
    const summary={
      status:'SUCCESS',evaluation:'B3_1000_AGGREGATE',sampling_version:VERSION,engine_version:run.engine_version,
    ...
    console.log('[ARAM Rating v0.3.2] B3 1000 aggregate evaluation complete',summary);return summary;
  }
  window.aramRatingB3EvaluateV032={version:VERSION,evaluate};
  console.log('[ARAM Rating v0.3.2] B3 evaluator ready. READ ONLY · requires preloaded reference engine · run: await aramRatingB3EvaluateV032.evaluate()');
})();
