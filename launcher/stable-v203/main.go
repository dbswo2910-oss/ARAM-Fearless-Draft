...
func promoteOnly(local, self, appDir, expected string) error {
	...
	if !selected.Valid {
		return fmt.Errorf("promotion app candidate invalid: %s", selected.Reason)
	}
	if expected != "" && compareVersion(selected.Version, expected) != 0 {
		return fmt.Errorf("promotion version mismatch: installed=%s expected=%s", selected.Version, expected)
	}
	canonical, err := ensureCanonicalLauncher(local, self)
	if err != nil {
		return fmt.Errorf("canonical launcher promotion failed: %w", err)
	}
	shortcuts := scanShortcutsWindows()
	repairs := repairProductShortcuts(shortcuts, canonical)
	if err := writeActiveInstall(local, canonical, selected); err != nil {
		return fmt.Errorf("active install pointer failed: %w", err)
	}
	cwd, _ := os.Getwd()
...
func main() {
	...
	if hasArg("--promote-only") {
		if err := promoteOnly(local, self, argValue("--app-dir"), argValue("--expected-version")); err != nil {
			d := LauncherDiagnostic{Schema: 1, At: time.Now().UTC().Format(time.RFC3339Nano), LauncherVersion: launcherVersion, LauncherExecutable: self, LauncherSHA256: fileSHA256(self), LocalAppData: local, BootSource: "in-app-update-promotion"}
			fail(local, d, err)
			os.Exit(2)
		}
		return
	...
	writeDiagnostic(local, d)
	pointerLauncher := canonical
	if canonicalErr != nil || pointerLauncher == "" {
		pointerLauncher = self
	}
