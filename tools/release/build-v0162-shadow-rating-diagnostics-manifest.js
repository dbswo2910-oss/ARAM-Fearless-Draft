...
function apply(base){
  ...
  out.version='0.16.2';
  out.message='v0.16.2 · SHADOW RATING DIAGNOSTICS';
...
function validate(m){
  if(String(m.version)!=='0.16.2')throw new Error(`expected v0.16.2 manifest, got ${m.version}`);
...
function main(){
  ...
  if(!['0.16.1','0.16.2'].includes(String(current.version)))throw new Error(`refusing manifest transition from ${current.version}`);
  const out=apply(current);
  validate(out);
  const text=JSON.stringify(out,null,2)+'\n';
  if(fs.readFileSync(manifestPath,'utf8')!==text)fs.writeFileSync(manifestPath,text,'utf8');
  console.log('V0.16.2 SHADOW RATING DIAGNOSTICS MANIFEST: VALID',JSON.stringify({version:out.version,changed_entries:CHANGES.length,production_rating_active:false,automatic_rating_promotion:false,all_sources_under_update:true}));
}
