...
function validate(m){if(String(m.version)!=='0.16.1')throw new Error(`expected v0.16.1 manifest, got ${m.version}`);if(m.production_rating_active!==false||m.automatic_rating_promotion!==false||m.universal_rating_shadow!==true)throw new Error('shadow safety metadata invalid');for(const row of m.files||[])if(!String(row.source||'').startsWith('update/'))throw new Error(`unsafe manifest source ${row.path} -> ${row.source}`);const byPath=new Map((m.files||[]).map(x=>[String(x.path),x]));for(const e of expectedEntries()){const got=byPath.get(e.path);if(!got)throw new Error(`manifest missing ${e.path}`);if(got.source!==e.source)throw new Error(`source mismatch ${e.path}`);if(String(got.sha256||'').toLowerCase()!==e.sha256)throw new Error(`sha256 mismatch ${e.path}`)}return true}
function main(){const current=JSON.parse(fs.readFileSync(manifestPath,'utf8'));let out;if(String(current.version)==='0.16.0')out=apply(current);else if(String(current.version)==='0.16.1')out=apply(current);else throw new Error(`refusing manifest transition from ${current.version}`);validate(out);const text=JSON.stringify(out,null,2)+'\n';if(fs.readFileSync(manifestPath,'utf8')!==text)fs.writeFileSync(manifestPath,text,'utf8');console.log('V0.16.1 UNIVERSAL RATING SHADOW MANIFEST: VALID',JSON.stringify({version:out.version,changed_entries:CHANGES.length,production_rating_active:false,automatic_rating_promotion:false,all_sources_under_update:true}));}
