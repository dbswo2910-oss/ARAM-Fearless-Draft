...
ok('live hierarchy has threat TOP3 contract',code.includes('>위협 TOP3<')&&code.includes('threatTop3HtmlV01582'));
...
try{
  ...
  const model={
    ...
    threats:[{name:'징크스',score:94,damageType:'물리'},{name:'말파이트',score:81,damageType:'마법'},{name:'오리아나',score:76,damageType:'마법'}],
    ...
    job:'적 첫 진입을 받아친 뒤 2차 진입',matchup:'우리 약우세',power:{our:69,enemy:64,diff:5},
...
const report={version:'0.15.82+',active_version:manifest.version,generated_at:new Date().toISOString(),pass:checks.filter(x=>x.pass).length,fail:checks.filter(x=>!x.pass).length,status:checks.every(x=>x.pass)?'PASS':'FAIL',checks,info:{purpose:'verify the v0.15.82 IN GAME command-center contract remains intact in the active successor',score_logic_changed:false,item_recommendation_logic_changed:false,ingame_hud_changed:true}};
