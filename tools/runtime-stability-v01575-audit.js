...
const pkg=JSON.parse(src('package.json')||'{}'),entryTarget=String(pkg.main||''),entry=src('main.js'),runtimePath=resolveCurrentRuntimeSource(ROOT,m),runtime=read(runtimePath),v79Entry=src('main-v01579.js'),patchSrc=src('ingame-transition-patch-v01575.js');
...
for(const [n,s] of [['current entry',entry],['effective runtime',runtime],['v0.15.79 safety lineage',v79Entry],['historical v75 entry',historicalEntry],['historical v75 preload',historicalPreload],['historical v75 watchdog',historicalWatch],['transition patch',patchSrc]]){try{new Function(s);ok(n+' parses',true)}catch(e){ok(n+' parses',false,e.message)}}
ok('manifest is v0.15.75 or newer',ge(m.version,'0.15.75'),m.version);ok('package is v0.15.75 or newer',ge(pkg.version,'0.15.75'),pkg.version||'');ok('current package entry delivered',entryTarget==='main.js'&&!!by.get('main.js')&&!!entry,`${entryTarget} -> ${by.get('main.js')||'missing'}`);
...
ok('historical safety root patches installed index before core/main compile',v79Entry.includes("require('./ingame-transition-patch-v01575')")&&v79Entry.indexOf('patchInstalledIndex')>0&&v79Entry.indexOf('patchInstalledIndex')<v79Entry.indexOf("const autosyncCore=require('./autosync-core')"));
if(clean){
  ok('clean entry records v0.15.79 safety root without runtime wrapper chain',entry.includes("legacySafetyRoot:'main-v01579.js'")&&entry.includes('flattened:true')&&entry.includes('runtimeSuccessorWrappers:false'));
  const prepAt=runtime.indexOf('prepareBaseUiForRuntime();'),loadAt=runtime.indexOf('mainWindow.loadFile(');
  ok('flattened runtime preserves index preparation before loadFile',prepAt>=0&&loadAt>prepAt,`${prepAt}/${loadAt}`);
  ok('clean active runtime has no successor compilation',!entry.includes('module._compile(')&&!runtime.includes('module._compile('));
}
...
ok('score logic untouched by clean entry and transition patch',!entry.includes('teamScore(')&&!entry.includes('recommendPicks(')&&!entry.includes('recommendBans(')&&patchSrc.includes('score_logic_changed:false'));
const report={version:'0.15.75-historical',checks,pass:checks.every(x=>x.pass),score_logic_changed:false,forwardCompatible:true,effectiveRuntime:runtimePath};fs.mkdirSync(path.join(ROOT,'audit-output'),{recursive:true});fs.writeFileSync(path.join(ROOT,'audit-output/runtime-stability-v01575-report.json'),JSON.stringify(report,null,2));for(const c of checks)console.log(`${c.pass?'PASS':'FAIL'} ${c.name}${c.detail?' · '+c.detail:''}`);if(!report.pass)process.exit(1);
