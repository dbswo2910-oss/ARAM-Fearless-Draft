...
ok(main.includes("'random-ingame-coach-v01550.js','random-ingame-ux-v01551.js'"),'Effective runtime keeps coach -> v0.15.51 UX injection order');
ok(main.includes('__ARAM_RANDOM_INGAME_COACH_V01550__')&&main.includes('__ARAM_RANDOM_INGAME_UX_V01551__'),'Effective runtime readiness guard covers coach and v0.15.51 UX polish');
const mainVersion=(main.match(/const VERSION='([^']+)'/)||[])[1]||'';
ok(atLeast(mainVersion,'0.15.51'),'Effective runtime VERSION is v0.15.51 or newer',mainVersion);
ok(atLeast(pkg.version,'0.15.51'),'package.json VERSION is v0.15.51 or newer',pkg.version);
report.info={scope:'forward-compatible regression contract for screenshot-driven v0.15.51 visual hierarchy polish',scoreLogicChanged:false,effectiveRuntime:mainPath,changes:['hide pick-stage noise in ingame','compact preview toolbar','remove duplicate situation card','prioritize role and next purchase','prioritize optimized death build']};
