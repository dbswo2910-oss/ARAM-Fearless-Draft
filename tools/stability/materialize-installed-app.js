...
const base=arg('--base'),out=arg('--out'),manifestPath=arg('--manifest')||'update/manifest.json';
if(!base||!out)throw new Error('usage: node tools/stability/materialize-installed-app.js --base <base-app-dir> --out <out-dir> [--manifest update/manifest.json]');
...
const copied=[];
for(const f of manifest.files||[]){const target=safeRel(f.path,'manifest target'),source=safeRel(f.source,'manifest source'),src=path.resolve(root,source);if(!src.startsWith(root+path.sep)||!fs.existsSync(src))throw new Error(`manifest source missing: ${source}`);if(f.sha256&&sha(src)!==String(f.sha256).toLowerCase())throw new Error(`manifest sha mismatch: ${source}`);const dst=path.join(absOut,target);fs.mkdirSync(path.dirname(dst),{recursive:true});fs.copyFileSync(src,dst);copied.push({target,source,sha256:sha(dst)})}
...
const canonicalDst=path.join(absOut,'canonical-shadow','src');copyDir(path.join(root,'src'),canonicalDst);
...
const report={status:'SUCCESS',stage:'INSTALLED_APP_MATERIALIZED',golden_version:manifest.version,base_dir:path.basename(absBase),output_dir:path.basename(absOut),manifest_files:(manifest.files||[]).length,copied_files:copied.length,deleted_files:deleted.length,package_version:pkg.version,package_main:entry,canonical_shadow_files:canonicalFiles.length,canonical_shadow_embedded_for_test_only:true,production_manifest_unchanged:true,user_data_identity:'aram-fearless-draft',research_database:'aram-rating-research-v03',checkpoint_key:'checkpoint-v03',acceptance_scope:'materialized installed-like Golden app + canonical shadow payload; actual Windows Electron boot is performed by CI workflow'};
