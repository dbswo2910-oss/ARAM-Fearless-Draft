...
function versionAtLeast(a,b){const A=versionParts(a),B=versionParts(b),n=Math.max(A.length,B.length);for(let i=0;i<n;i++){const x=A[i]||0,y=B[i]||0;if(x!==y)return x>y}return true}
const manifest=JSON.parse(fs.readFileSync(manifestPath,'utf8'));
const activeVersion=String(manifest.version||'');
if(!versionAtLeast(activeVersion,'0.15.135'))throw new Error(`installed materializer requires Golden v0.15.135 or a validated successor, got ${activeVersion}`);
...
if(String(pkg.version)!==activeVersion)throw new Error(`assembled package version ${pkg.version} != manifest ${activeVersion}`);
...
const report={status:'SUCCESS',stage:'INSTALLED_APP_MATERIALIZED',golden_version:activeVersion,active_version:activeVersion,base_dir:path.basename(absBase),output_dir:path.basename(absOut),in_place_update:inPlace,manifest_files:(manifest.files||[]).length,copied_files:copied.length,git_blob_sources:gitBlobSources,deleted_files:deleted.length,package_version:pkg.version,package_main:entry,canonical_shadow_files:canonicalFiles.length,canonical_shadow_embedded_for_test_only:true,production_manifest_unchanged:true,user_data_identity:'aram-fearless-draft',research_database:'aram-rating-research-v03',checkpoint_key:'checkpoint-v03',acceptance_scope:'materialized installed-like current app + canonical shadow payload; manifest sources are copied from Git blobs when available so Windows CRLF checkout conversion cannot alter pinned bytes; --in-place is test-only and preserves the external stable userData profile while applying the authoritative manifest'};
