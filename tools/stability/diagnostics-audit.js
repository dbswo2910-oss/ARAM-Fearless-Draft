...
L.must(!/setInterval\s*\(|MutationObserver\s*\(/.test(panelSrc),'diagnostics panel must not create recurring repair/polling owners');
...
L.must(diagnostics.contract?.components?.collector==='shadow'&&diagnostics.contract?.components?.panel==='shadow'&&diagnostics.contract?.components?.production_menu_entry==='planned','diagnostics migration contract drift');
...
const report={status:'SUCCESS',schema:contract.schema,privacy:contract.privacy,read_only:true,stable_app_identity:'aram-fearless-draft',research_database:'aram-rating-research-v03',checkpoint_key:'checkpoint-v03',canonical_panel:{status:'shadow',stable_roles:roles,recurring_polling:false,copy_json:true},production_enabled:false,note:'Canonical diagnostics collector + panel are shadow-only; production v0.15.135 remains untouched.'};
L.write('audit-output/stability/diagnostics-report.json',report);
console.log('DIAGNOSTICS CONTRACT AUDIT: SUCCESS · collector and panel shadow ready');
