...
function parseArgs(argv) {
  ...
  let target = null, out = null, skipWalkForward = false;
  while (args.length) {
    const arg = args.shift();
    if (arg === '--target') target = String(args.shift() || '').trim() || null;
    else if (arg === '--out') out = String(args.shift() || '').trim() || null;
    else if (arg === '--skip-walk-forward') skipWalkForward = true;
...
function main() {
  ...
  if (parsed.help || !parsed.input) {
    ...
    process.exitCode = parsed.input ? 0 : 2;
    return;
  }
...
module.exports = { parseArgs, extractMatches, usage };
