...
const newItems="const {itemCatalog,updater}=canonicalOwners;";
...
if((baseline.split(oldItemIpc).length-1)!==1)throw new Error('frozen v0.17 item catalog IPC block drifted');
...
if((candidate.split(newItems).length-1)!==1)throw new Error('v0.18 canonical owner destructure cardinality != 1');
if((candidate.split(newItemIpc).length-1)!==1)throw new Error('v0.18 canonical Items IPC install cardinality != 1');
...
if(candidate.includes(oldItemIpc))throw new Error('legacy item IPC owner survived candidate');
...
const report={status:'PASS',baselineSha256:sha(baseline),candidateSha256:sha(candidate),compositionRoot:'src/app/canonical-runtime-owners.js',approvedChanges:{version:'0.17.0 -> 0.18.0',legacyAutoSyncRequiresRemoved:4,legacyItemCatalogRequiresRemoved:1,legacyItemIpcBlockRemoved:1,legacyUpdaterSafetyRequiresRemoved:4,canonicalCompositionRootAdded:1,canonicalAutoSyncInstallAdded:1,canonicalItemsIpcInstallAdded:1},unapprovedDrift:false};
