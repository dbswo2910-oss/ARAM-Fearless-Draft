...
const entry=src(pkg.main||''),preload=src('preload.js'),safe=src('update-safety-v01579.js'),up=src('updater-safety-patch-v01579.js'),rt=src('runtime-safety-net-v01579.js'),loader=src('runtime-loader-v01579.js'),hb=src('hang-heartbeat-v01579.js'),wd=src('external-watchdog-v01579.js'),stab=src('runtime-source-stability-v01579.js');
for(const [name,code] of [['entry',entry],['preload',preload],['update safety',safe],['updater patch',up],['runtime safety',rt],['loader',loader],['heartbeat',hb],['watchdog',wd],['source stability',stab]]){try{new Function(code);ok(name+' parses',true)}catch(e){ok(name+' parses',false,e.message)}}
ok('manifest version',manifest.version==='0.15.79',manifest.version);
ok('package version',pkg.version==='0.15.79',pkg.version||'');
ok('package entry',pkg.main==='main-v01579.js',pkg.main||'');
...
ok('boot guard runs before normal runtime compile',entry.indexOf('installBootGuard')>0&&entry.indexOf('installBootGuard')<entry.indexOf("require('./autosync-live-runtime-v01571')"));
ok('v78 freeze fix remains before base compile',entry.includes('patchInGame(before)')&&entry.includes('patchRenderAll(a.text)')&&entry.indexOf('patchInstalledIndex()')<entry.indexOf("const autosyncCore=require('./autosync-core')"));
ok('stable dual heartbeat installed',entry.includes('hang-heartbeat-v01579')&&preload.includes('heartbeat-renderer.json')&&hb.includes('heartbeat-main.json'));
ok('safety net loads before performance owners',entry.includes("'runtime-blackbox-v01577.js','runtime-safety-net-v01579.js','runtime-performance-v01568.js'"));
...
ok('updater is transaction wrapped',entry.includes('patchUpdaterSource(src)')&&up.includes('prepareUpdateTransaction')&&up.includes('markUpdateApplied')&&up.includes('abortUpdateTransaction'));
...
if(patcher){
  ...
  const offenders=[];
  for(const [file,source] of by){if(!file.endsWith('.js')||historicalAllow.has(file)||nodeOnly.test(file))continue;try{const patched=patcher(file,read(source));if(/\.observe\(document\.(?:documentElement|body)/.test(patched))offenders.push(file)}catch(e){offenders.push(file+':PATCH_ERROR')}}
...
ok('safety baseline is scoring neutral',entry.includes("replaceAll('0.15.70','0.15.79')")&&!entry.includes('teamScore(')&&!entry.includes('recommendPicks(')&&!entry.includes('recommendBans(')&&safe.includes('score_logic_changed:false')&&stab.includes('score_logic_changed:false'));
const report={version:'0.15.79',checks,pass:checks.every(x=>x.pass),score_logic_changed:false,info:{purpose:'permanent safety baseline for future patches: transaction rollback, probation, hot-path circuit breakers and release-gate policy'}};
