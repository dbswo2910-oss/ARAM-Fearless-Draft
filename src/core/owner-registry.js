...
const owners=Object.freeze({
  main:{canonical:'src/main',legacy:'update/v0.15.135/main-v015135.js',status:'planned'},
  preload:{canonical:'src/preload',legacy:'update/v0.15.117/preload.js',status:'planned'},
  state:{canonical:'src/state',legacy:'state-integrity-v015117',status:'planned'},
...
function assertSingleOwner(){
  ...
  for(const [name,x] of Object.entries(owners)){
    if(canonical.has(x.canonical))throw new Error(`duplicate canonical owner path ${x.canonical}: ${canonical.get(x.canonical)}, ${name}`);
    canonical.set(x.canonical,name);
  }
  return true;
}
