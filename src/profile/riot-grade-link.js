...
function matchChampionId(match){for(const root of [match,match?.me,match?.participant,match?.localParticipant,match?.data,match?.info]){const n=Number(root?.championId??root?.championID);if(Number.isFinite(n))return n}return null}
...
function findAuthoritativeRecord(state,match,{puuid='',championId=null}={}){
  const gid=riot.canonGameId(match?.gameId??match?.id??match?.matchId??match?.metadata?.matchId),targetPuuid=String(puuid||matchPuuid(match)).trim(),cid=Number.isFinite(Number(championId))?Number(championId):matchChampionId(match);
