...
function createSelectionState({normalizeName=(v=>String(v||'').trim()),canonicalName=((text)=>String(text||'').trim()),text=(el=>String(el?.textContent||'').trim())}={}){
  ...
  function snapshot(results,options={}){
    if(!results)return null;const selected=normalizeName(results.dataset.selectedCandidate||'');if(!selected)return null;const rows=[...(results.querySelectorAll?.('.combo')||[])].slice(0,5);const row=rows.find(x=>normalizeName(x.dataset?.rp93Candidate||canonicalName(text(x.querySelector?.('.names')),x))===selected)||null;
    ...
    const name=row.dataset?.rp93Candidate||selected,ad=Number(row.dataset?.rp93Ad),ap=Number(row.dataset?.rp93Ap),score=text(row.querySelector?.('.comboScore'))||text(row.querySelector?.('.rp90Score'))||results.dataset.selectedCandidateScore||'-',desc=text(row.querySelector?.('.desc'))||text(row.querySelector?.('.rp90Desc'))||results.dataset.selectedCandidateDesc||'선택 후보 기준 조합 미리보기입니다.';return{row,name,profile:{adPct:Number.isFinite(ad)?ad:50,apPct:Number.isFinite(ap)?ap:50},score,desc}
  }
  return{clear,write,snapshot};
}
