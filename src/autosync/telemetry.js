'use strict';
/**
 * Canonical AutoSync telemetry owner.
 * Behavior-preserving migration of the v0.15.34 Golden implementation.
 * The semantic owner name is stable; legacy readiness flags remain temporary compatibility aliases.
 */
const OWNER='autosync.telemetry';
function install(mod){
  if(!mod||mod.__ARAM_AUTOSYNC_TELEMETRY__)return mod;
  // Preserve idempotence when this canonical owner meets an already-patched v0.17 runtime.
  if(mod.__ARAM_AUTOSYNC_TELEMETRY_V01534__){mod.__ARAM_AUTOSYNC_TELEMETRY__=true;return mod}
  ...
  function touch(gid,pid,raw){
    ...
    row.set(pid,raw);cache.delete(gid);cache.set(gid,row);
  ...
  mod.aramTelemetry={sanitize,scan,cache,leafCount,owner:OWNER};
  // Temporary compatibility surface for frozen v0.17 audits/readiness checks.
  mod.aramTelemetryV01534=mod.aramTelemetry;
  mod.__ARAM_AUTOSYNC_TELEMETRY__=true;
