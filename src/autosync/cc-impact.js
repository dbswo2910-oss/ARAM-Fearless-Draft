'use strict';
/** Canonical AutoSync CC-impact normalizer; behavior-preserving migration of Golden v0.15.25. */
const OWNER='autosync.cc-impact';
function install(coreMod){
  if(!coreMod)return coreMod;
  if(coreMod.__ARAM_AUTOSYNC_CC_IMPACT__)return coreMod;
  if(coreMod.__ARAM_CC_IMPACT_NORMALIZER_V01525__){coreMod.__ARAM_AUTOSYNC_CC_IMPACT__=true;return coreMod}
  ...
  function extract(raw){
    if(!raw)return null;
    const out={enemyChampionImmobilizations:read(raw,'enemyChampionImmobilizations'),immobilizeAndKillWithAlly:read(raw,'immobilizeAndKillWithAlly'),knockEnemyIntoTeamAndKill:read(raw,'knockEnemyIntoTeamAndKill'),highestCrowdControlScore:read(raw,'highestCrowdControlScore')};
  ...
  coreMod.normalizeAramHistoryGame=function(game,target,champName){
    ...
    try{
      ...
      if(cc&&out.me&&typeof out.me==='object'){
        out.me.ccImpactRaw=cc;for(const k of cc.availableKeys)if(out.me[k]===undefined)out.me[k]=cc[k];out.ccImpactRawAvailable=true;
      }else out.ccImpactRawAvailable=false;
    }catch(e){out.ccImpactRawAvailable=false;out.ccImpactRawError=e?.message||String(e)}
    return out;
  };
  coreMod.__ARAM_AUTOSYNC_CC_IMPACT__=true;
