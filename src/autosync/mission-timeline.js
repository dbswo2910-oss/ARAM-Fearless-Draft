'use strict';
/** Canonical AutoSync mission-timeline owner; behavior-preserving migration of Golden v0.15.29. */
const OWNER='autosync.mission-timeline';
function install(coreMod){
  const Core=coreMod?.LeagueAutoSyncCore;
  if(!Core)return coreMod;
  if(Core.prototype.__ARAM_AUTOSYNC_MISSION_TIMELINE__)return coreMod;
  if(Core.prototype.__ARAM_MISSION_TIMELINE_V01529__){Core.prototype.__ARAM_AUTOSYNC_MISSION_TIMELINE__=true;return coreMod}
  ...
  function frames(root){if(Array.isArray(root?.frames))return root.frames;if(Array.isArray(root?.timeline?.frames))return root.timeline.frames;if(Array.isArray(root?.data?.frames))return root.data.frames;if(Array.isArray(root))return root;return[]}
  function allEvents(root){const out=[];for(const f of frames(root))for(const e of arr(f?.events))out.push(e);for(const e of arr(root?.events))out.push(e);return out.sort((a,b)=>eventTime(a)-eventTime(b))}
  function participantSets(m){const our=new Set(arr(m?.ourTeam).map(pid).filter(x=>x!==null));const enemy=new Set(arr(m?.enemyTeam).map(pid).filter(x=>x!==null));const me=pid(m?.me);return{our,enemy,me,complete:me!==null&&our.size>=5&&enemy.size>=5}}
  function isOurKill(e,sets){const k=num(e?.killerId),v=num(e?.victimId);return k!==null&&v!==null&&sets.our.has(k)&&sets.enemy.has(v)}
  function isEnemyKill(e,sets){const k=num(e?.killerId),v=num(e?.victimId);return k!==null&&v!==null&&sets.enemy.has(k)&&sets.our.has(v)}
  ...
  function summarize(root,m){
    ...
    for(const d of deaths){
      ...
      const mePrior=before.filter(e=>meTakedown(e,sets)).length,allyBefore=before.filter(e=>isOurKill(e,sets)).length,enemyBefore=before.filter(e=>isEnemyKill(e,sets)).length,allyAfter=after.filter(e=>isOurKill(e,sets)).length,enemyAfter=after.filter(e=>isEnemyKill(e,sets)).length;
      const netOther=(allyBefore+allyAfter)-(enemyBefore+enemyAfter),postDeathByMe=after.filter(e=>meTakedown(e,sets)).length;
      const productive=(mePrior>0&&netOther>=0)||(allyAfter>0&&netOther>0)||postDeathByMe>0,strong=(mePrior>0&&allyAfter>0)||(allyAfter>=2&&netOther>=1)||postDeathByMe>0||netOther>=2;
  ...
  async function getSummary(core,m){
    ...
    if(cache.has(gid))return cache.get(gid);let out;
  ...
  Core.prototype.getAramMatchHistory=async function(opts={}){
    ...
    await enrich(this,out.matches);out.timelineCoverage=out.matches.filter(m=>m?.missionTimelineV01529?.available).length;out.timelineRequested=Math.min(out.matches.length,40);return out;
  };
  Core.prototype.__ARAM_AUTOSYNC_MISSION_TIMELINE__=true;
  Core.prototype.__ARAM_MISSION_TIMELINE_V01529__=true;
  coreMod.summarizeMissionTimeline=summarize;coreMod.summarizeMissionTimelineV01529=summarize;
  coreMod.__ARAM_AUTOSYNC_MISSION_TIMELINE__=true;coreMod.__ARAM_MISSION_TIMELINE_PATCH_V01529__=true;
  return coreMod;
}
module.exports={OWNER,install,patch:install,production_active:true,legacy_compatibility_aliases:true};
