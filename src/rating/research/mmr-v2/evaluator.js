...
// Kept for simple diagnostics/backward-compatible tests. Model selection does NOT
// use this two-way split; evaluateMmrV2 uses temporalThreeWaySplit below.
...
function temporalThreeWaySplit(matches, { trainFraction = 0.6, validationFraction = 0.2 } = {}) {
  ...
  if (rows.length < 3) {
    return Object.freeze({
      train: Object.freeze(rows),
      validation: Object.freeze([]),
      test: Object.freeze([]),
      trainEnd: rows.at(-1)?.timestamp ?? null,
  ...
  const train = rows.slice(0, trainCut);
  const validation = rows.slice(trainCut, validationCut);
  const test = rows.slice(validationCut);
  return Object.freeze({
...
function metricsOnly(summary) {
  return Object.freeze({
    ...
    frozenCohorts: summary.frozenCohorts,
    walkForward: summary.walkForward,
    walkForwardCohorts: summary.walkForwardCohorts
  });
}
...
function championCoverage(matches) {
  ...
  for (const match of matches || []) {
    for (const id of [...(match.teamAChampions || []), ...(match.teamBChampions || [])]) {
      total += 1;
      if (id != null) known += 1;
    }
  }
...
function validationGate(candidateSummary, referenceSummary, validationLength, bootstrapIterations, seed, options = {}) {
  ...
  return Object.freeze({
    ...
    minValidationMatches,
    bootstrap,
    nonRegression: regression,
...
function selectModelsOnValidation(split, options, bootstrapIterations) {
  ...
  if (bestRecency) {
    ...
    featureGates.recency = Object.freeze({ candidate: null, reference: selected.id, eligible: false, accepted: false, reason: 'no_recency_candidates' });
  }
  ...
  if (bestChampion) {
    ...
    featureGates.champion = Object.freeze({
      candidate: bestChampion.id,
      reference: selected.id,
      championCoverage: coverage,
      minChampionCoverage,
      ...gate,
      eligible: gate.eligible && coverageEligible,
      accepted: gate.accepted && coverageEligible
    });
    if (gate.accepted && coverageEligible) selected = bestChampion;
  } else {
    featureGates.champion = Object.freeze({ candidate: null, reference: selected.id, championCoverage: coverage, minChampionCoverage, eligible: false, accepted: false, reason: 'no_champion_candidates' });
  }
  ...
  return Object.freeze({
    ...
    selectedBaseline,
    selectedV2: selected.id,
    selectedV2Options: Object.freeze({ ...selected.modelOptions }),
    validation: Object.freeze({
      baselines: Object.freeze(baselineValidation),
      candidates: Object.freeze(candidateValidation)
    }),
...
function finalEvaluation(split, selection, options, bootstrapIterations) {
  ...
  const baseline = summarizeModel(baselineFrozenRows, baselineWalkRows);
  const candidate = summarizeModel(v2Frozen.rows, v2Walk.rows);
  const bootstrap = pairedBootstrapDiff(candidate.predictions, baseline.predictions, {
    metric: 'logLoss',
    iterations: bootstrapIterations,
    seed: 0x56324241
  });
  ...
  return Object.freeze({
    selectedBaseline: selection.selectedBaseline,
    selectedV2: selection.selectedV2,
    baseline: metricsOnly(baseline),
    candidate: metricsOnly(candidate),
    v2VsBaseline: bootstrap,
    v2NonRegression: regression
  });
}
...
function evaluateMmrV2(rawMatches, options = {}) {
  ...
  const selection = split.validation.length
    ? selectModelsOnValidation(split, options, bootstrapIterations)
    : Object.freeze({
      selectionSource: 'validation_only',
      finalTestUsedForSelection: false,
      selectedBaseline: null,
      selectedV2: null,
      selectedV2Options: null,
      validation: Object.freeze({ baselines: Object.freeze({}), candidates: Object.freeze({}) }),
      featureGates: Object.freeze({})
    });
  const finalTest = selection.selectedBaseline && selection.selectedV2 && split.test.length
    ? finalEvaluation(split, selection, options, bootstrapIterations)
    : null;
  ...
  const minValidationMatches = Number.isInteger(options.minValidationMatches) ? options.minValidationMatches : 100;
  const dataGatePassed = dataset.acceptedCount >= minMatches && split.validation.length >= minValidationMatches && split.test.length >= minFrozenTest;
  let decision = 'no_clear_winner';
  let reason = 'predictive_advantage_not_established';
  if (!dataGatePassed) {
    decision = 'insufficient_real_data';
    reason = `requires_at_least_${minMatches}_matches_${minValidationMatches}_validation_${minFrozenTest}_untouched_test`;
  ...
  const firstTimestamp = dataset.matches[0]?.timestamp ?? null;
  const lastTimestamp = dataset.matches.at(-1)?.timestamp ?? null;
  ...
  return Object.freeze({
    ...
    temporal: Object.freeze({
      scheme: 'nested_temporal_60_20_20_default',
      trainMatches: split.train.length,
      validationMatches: split.validation.length,
      frozenTestMatches: split.test.length,
      finalTestMatches: split.test.length,
      ...
      validationFingerprint: evidenceFingerprint(split.validation),
      testFingerprint: evidenceFingerprint(split.test)
    }),
    selection,
    finalTest,
    selectedV2: selection.selectedV2,
    bestBaseline: selection.selectedBaseline,
    v2VsBaseline: finalTest?.v2VsBaseline || null,
    v2NonRegression: finalTest?.v2NonRegression || null,
    promotionGate: Object.freeze({
      ...
      decision,
      reason,
      candidate: decision === 'candidate_model' ? selection.selectedV2 : null,
      automaticPromotion: false
    })
  });
}
...
module.exports = {
  BASELINES,
  DEFAULT_STATIC_PLAYER_GRID,
  DEFAULT_RECENCY_HALF_LIVES,
  DEFAULT_CHAMPION_LAMBDAS,
  temporalSplit,
