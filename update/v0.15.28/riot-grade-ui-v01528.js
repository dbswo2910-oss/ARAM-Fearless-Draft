...
(()=>{
  ...
  function targetPuuid(){return String(aramHistoryState?.account?.puuid||aramHistoryState?.localAccount?.puuid||'')}
  function findRecord(s,m){const gid=canon(m?.gameId),puuid=targetPuuid();return (s?.records||[]).find(r=>canon(r.gameId)===gid&&(!puuid||!r.puuid||String(r.puuid)===puuid))||null}
  ...
  async function annotate(force=false){
    try{style();const root=document.getElementById('historyMatchDetail'),m=aramHistoryState?.matches?.find(x=>String(x.gameId)===String(aramHistoryState.selectedGameId));if(!root||!m)return;const s=await state(force);root.querySelector('#riotGradeCheckV01528')?.remove();const wrap=document.createElement('div');wrap.id='riotGradeCheckV01528';wrap.innerHTML=card(m,s);const tabs=root.querySelector('.matchLabTabs');if(tabs?.parentNode)tabs.insertAdjacentElement('afterend',wrap);else root.prepend(wrap)}catch(e){console.warn('[v0.15.28] Riot Grade UI annotate failed',e)}
  }
