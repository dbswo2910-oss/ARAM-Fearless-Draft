...
function createCollector(core,{app,intervalMs=5000}={}){
  ...
  async function poll(){
    ...
    try{
      ...
      let fallbackGameId='';
      if(rows.some(r=>!r.gameId)){
        try{const eog=await core.lcuGet(EOG_ENDPOINT,3000);fallbackGameId=canonGameId(deepFirst(eog,['gameId','gameID']))}catch{}
      }
      const a=account(),now=Date.now();let added=0;
      for(const r0 of rows){
        const r={...r0};if(!r.gameId&&fallbackGameId){r.gameId=fallbackGameId;r.gameIdSource='eog-stats-fallback'}else r.gameIdSource=r.gameId?'mastery-update':'unlinked';
        ...
        const i=records.findIndex(x=>x.key===rec.key||(rec.gameId&&canonGameId(x.gameId)===rec.gameId&&num(x.championId)===num(rec.championId)&&x.grade===rec.grade&&(!a.puuid||!x.puuid||x.puuid===a.puuid)));
      ...
      records=records.sort((a,b)=>num(b.capturedAt,0)-num(a.capturedAt,0)).slice(0,MAX_RECORDS);status.records=records.length;status.lastCaptureAt=added?now:status.lastCaptureAt;status.lastResult=added?`Riot Grade ${added}건 저장`:`기존 Riot Grade 재확인`;status.lastError='';if(added)save();
