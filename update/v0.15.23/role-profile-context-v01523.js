...
`;document.head.appendChild(s)}
  ...
  function postProcess(){
    ...
    const idx=root.querySelector('.ppidx');if(idx&&selectedRole){const sm=idx.querySelector(':scope > small');if(sm)sm.textContent=`${selectedRole} PLAYER INDEX`;sm?.classList.add('pp23roleIndex');const p=api.profile();const allN=fullMatches().length,roleN=p?.n||0;const notes=[...idx.querySelectorAll(':scope > small')];const last=notes[notes.length-1];if(last&&last!==sm)last.textContent=`${selectedRole} 분석 ${roleN}/${allN}경기 · 신뢰도 ${roleN>=12?'높음':roleN>=6?'보통':roleN>=3?'낮음':'표본 부족'} · ${p?.wins||0}승 ${Math.max(0,roleN-(p?.wins||0))}패`;const typeSmall=idx.querySelector('.pp21type small');if(typeSmall)typeSmall.textContent=`${selectedRole} 역할 ${roleN}경기 · 역할 내부 플레이 성향`}
    if(selectedRole){const p=api.profile(),a=window.aramPlayerProfileDiversityV01521?.analyze?.(p);if(a?.primaryType){const ranked=Object.entries(a.style||{}).map(([k,v])=>({k,v:num(v)})).sort((x,y)=>y.v-x.v),names={aggression:'교전 적극성',survival:'생존 지향',carry:'캐리 지향',frontline:'전열 성향',control:'제어·지원',consistency:'안정성'},hi=ranked[0],lo=ranked[ranked.length-1],scout=root.querySelector('.ppscout');if(scout)scout.textContent=`${selectedRole} 전적 기준 ${a.primaryType.label} 성향입니다. ${names[hi?.k]||'강점 축'}이 가장 두드러지고 ${names[lo?.k]||'보완 축'}은 상대적으로 낮습니다. 이 평가는 ${p.n}경기의 ${selectedRole} 전적만 사용합니다.`}const grids=root.querySelectorAll('.ppgrid');const lastGrid=grids[grids.length-1],coachBox=lastGrid?.querySelectorAll('.ppbox')?.[1];if(coachBox)coachBox.innerHTML=roleCoach(selectedRole)}
    syncDrill();
    return true;
  }
