...
`;document.head.appendChild(s)}
  ...
  function candidate(){
    const els=[...document.querySelectorAll('button,[role="button"],span,div')].filter(el=>{
      const t=String(el.textContent||'').replace(/\s+/g,' ').trim();
      if(t.length<3||t.length>38)return false;
      if(!/(업데이트|최신\s*버전)/.test(t))return false;
      if(el.children.length&&[...el.children].some(c=>/(업데이트|최신\s*버전)/.test(String(c.textContent||''))))return false;
      const r=el.getBoundingClientRect();return r.width>45&&r.width<260&&r.height>18&&r.height<70;
    });
    return els.sort((a,b)=>a.getBoundingClientRect().top-b.getBoundingClientRect().top||a.getBoundingClientRect().width-b.getBoundingClientRect().width)[0]||null;
  }
  ...
  const mo=new MutationObserver(()=>{if(!lastBound||!document.contains(lastBound))lastBound=null;bind()});mo.observe(document.documentElement,{subtree:true,childList:true,characterData:true});
