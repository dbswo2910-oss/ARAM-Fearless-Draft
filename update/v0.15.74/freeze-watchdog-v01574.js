...
function install(opts={}){
  ...
  app.on('browser-window-created',(_e,win)=>{
    ...
    const installRendererTrace=()=>wc.executeJavaScript(`(()=>{if(window.__ARAM_FREEZE_TRACE_V01574__)return true;window.__ARAM_FREEZE_TRACE_V01574__={last:{type:'boot',at:Date.now()}};const mark=(type,e)=>{let t=e?.target,desc='';try{desc=[t?.id||'',t?.className||'',t?.textContent||''].join(' ').replace(/\\s+/g,' ').trim().slice(0,160)}catch{}window.__ARAM_FREEZE_TRACE_V01574__.last={type,desc,at:Date.now()}};document.addEventListener('click',e=>mark('click',e),true);document.addEventListener('change',e=>mark('change',e),true);document.addEventListener('input',e=>mark('input',e),true);return true})()`,false).catch(()=>{});
    const probeCode=`(()=>{const active=[...document.querySelectorAll('#nav .tab.active,#random.active,#data.active')].map(x=>x.id||x.textContent?.trim()?.slice(0,40));return{at:Date.now(),active,last:window.__ARAM_FREEZE_TRACE_V01574__?.last||null,random:window.aramRandomPracticeRuntimeV01572?.getStats?.()||null,ingame:window.aramRandomIngameRuntimeV01570?.getStats?.()||null,live:window.aramLiveAutosyncRuntimeV01571?.getStats?.()||null,icons56:window.aramRandomItemIconsV01556?.getStats?.()||null,shop53:window.aramRandomIngameShopV01553?.getStats?.()||null,icons57:window.aramItemIconsGlobalV01557?.getStats?.()||null,art66:window.aramItemArtResolverV01566?.getStats?.()||null,memory:performance?.memory?{used:performance.memory.usedJSHeapSize,total:performance.memory.totalJSHeapSize,limit:performance.memory.jsHeapSizeLimit}:null}})()`;
    const startProbe=()=>{clearInterval(probeTimer);probeTimer=setInterval(async()=>{if(wc.isDestroyed())return;try{const timeout=new Promise(r=>setTimeout(()=>r({__timeout:true}),1400));const v=await Promise.race([wc.executeJavaScript(probeCode,false),timeout]);if(v?.__timeout){const n=Date.now();if(n-lastTimeoutAt>8000){lastTimeoutAt=n;write('probe-timeout',{lastProbe,metrics:app.getAppMetrics?.()||[]})}}else lastProbe=v}catch(e){write('probe-error',{message:e?.message||String(e),lastProbe})}},2500)};
