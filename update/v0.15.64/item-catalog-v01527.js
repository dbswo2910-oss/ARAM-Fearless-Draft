...
function getJson(url,redirects=0){
  return new Promise((resolve,reject)=>{
    ...
    const req=https.get(u,{headers:{'User-Agent':'ARAM-Fearless-Draft/0.15.64','Cache-Control':'no-cache'}},res=>{
      ...
      if(code>=300&&code<400&&res.headers.location){res.resume();if(redirects>=3){reject(new Error('아이템 데이터 redirect 초과'));return}let next;try{next=new URL(res.headers.location,u).toString()}catch(e){reject(e);return}getJson(next,redirects+1).then(resolve,reject);return}
...
function communityIconMap(json){const out={};for(const row of Array.isArray(json)?json:[]){const id=String(Number(row?.id)||'');if(!id)continue;const url=currentGameIconUrl(row?.iconPath);if(url)out[id]=url}return out}
function communityFallbackMap(json){const out={};for(const row of Array.isArray(json)?json:[]){const id=String(Number(row?.id)||'');if(!id)continue;const url=clientPluginIconUrl(row?.iconPath);if(url)out[id]=url}return out}
function isStandardLiveItemId(id){const n=Number(id);return Number.isFinite(n)&&n>0&&n<10000}
...
function compact(version,json,currentIcons={},pluginIcons={}){
  ...
  for(const [id,it] of Object.entries(data)){
    ...
    const primary=currentIcons[String(id)]||'',plugin=pluginIcons[String(id)]||'',dd=ddragonIconUrl(version,id);
    ...
    items[String(id)]={id:String(id),name,total,base:base||(!from.length?total:0),sell,from,into,full,map12,purchasable,depth,tags,
      standardLiveId:isStandardLiveItemId(id),iconUrl:primary||plugin||dd,iconPrimaryUrl:primary,iconFallbackUrls:fallbacks,
  ...
  const all=Object.values(items),aramAll=all.filter(x=>x.map12&&x.purchasable),aram=aramAll.filter(x=>x.standardLiveId),legacyAlternate=aramAll.filter(x=>!x.standardLiveId);
  const countFor=rows=>({total:rows.length,primaryMapped:rows.filter(x=>!!x.iconPrimaryUrl).length,fallbackOnly:rows.filter(x=>!x.iconPrimaryUrl&&x.iconFallbackUrls.length>0).length,missing:rows.filter(x=>!x.iconUrl).length});
  const standard=countFor(aram),legacy=countFor(legacyAlternate);
  return{ok:true,version,locale:'ko_KR',items,count:Object.keys(items).length,loadedAt:Date.now(),source:'Data Dragon realms/kr · ko_KR',recipeAware:true,
    iconSource:'CommunityDragon latest /game/assets/items/icons2d current game art',iconFallback:'CommunityDragon client-plugin latest → Data Dragon current version',
    iconCount:all.filter(x=>!!x.iconPrimaryUrl).length,
    iconAudit:{aramPurchasable:aramAll.length,standardAramPurchasable:standard.total,standardPrimaryMapped:standard.primaryMapped,standardFallbackOnly:standard.fallbackOnly,standardMissing:standard.missing,legacyAlternateCount:legacy.total,legacyPrimaryMapped:legacy.primaryMapped,legacyFallbackOnly:legacy.fallbackOnly,legacyMissing:legacy.missing},
    cachePolicy:'v0.15.64 renderer cache-bust + legacy art flags invalidated'};
}
async function fetchItemJson(version,locale){return getJson(`https://ddragon.leagueoflegends.com/cdn/${encodeURIComponent(version)}/data/${locale}/item.json`)}
async function fetchCatalog(){
  ...
  pending=(async()=>{
    ...
    if(version){try{json=await fetchItemJson(version,'ko_KR')}catch(e){error=e?.message||String(e)}if(!json){try{json=await fetchItemJson(version,'en_US')}catch(e){error=e?.message||String(e)}}}
    if(!json){version=FALLBACK_VERSION;try{json=await fetchItemJson(version,'ko_KR')}catch(e){error=e?.message||String(e)}if(!json){try{json=await fetchItemJson(version,'en_US')}catch(e){error=e?.message||String(e)}}}
    cache=json?compact(version,json,currentIcons,pluginIcons):{ok:false,version:'',locale:'',items:{},count:0,loadedAt:Date.now(),source:'fallback',recipeAware:false,iconSource:'CommunityDragon latest /game/assets/items/icons2d current game art',iconCount:Object.keys(currentIcons).length,iconFallback:'CommunityDragon client-plugin latest → Data Dragon current version',iconAudit:{aramPurchasable:0,standardAramPurchasable:0,standardPrimaryMapped:0,standardFallbackOnly:0,standardMissing:0,legacyAlternateCount:0,legacyPrimaryMapped:0,legacyFallbackOnly:0,legacyMissing:0},error:error||realmError||'Data Dragon item catalog unavailable',iconError};
    if(cache?.ok&&iconError)cache.iconError=iconError;pending=null;return cache;
  })();return pending;
}
function register(ipcMain){if(registered)return;registered=true;ipcMain.handle('desktop:get-item-catalog',()=>fetchCatalog())}
module.exports={register,fetchCatalog,compact,communityIconMap,communityFallbackMap,currentGameIconUrl,clientPluginIconUrl,isStandardLiveItemId,get cache(){return cache}};
