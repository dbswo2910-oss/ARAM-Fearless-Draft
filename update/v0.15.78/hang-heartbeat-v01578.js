...
function install(opts={}){
  ...
  const file=path.join(dir,'heartbeat-main-v01578.json');
  const write=()=>{const body=JSON.stringify({at:Date.now(),pid:process.pid,version:'0.15.78'});fs.writeFile(file,body,()=>{})};
  write();timer=setInterval(write,500);timer.unref?.();
  try{
    child=spawn(process.execPath,[path.join(__dirname,'external-watchdog-v01578.js'),String(process.pid),dir],{env:{...process.env,ELECTRON_RUN_AS_NODE:'1'},detached:true,stdio:'ignore',windowsHide:true});
