...
const blackbox=require('./freeze-blackbox-v01577');
blackbox.install({version:'0.15.78'});
require('./hang-heartbeat-v01578').install({version:'0.15.78',record:blackbox.record});
...
function patchInstalledIndex(){
  ...
  const a=patchInGame(before);if(!a.ok)throw new Error('v0.15.78 in-game index contract mismatch: '+JSON.stringify(a.results));
  const b=patchRenderAll(a.text);if(!b.ok)throw new Error('v0.15.78 renderAll index contract mismatch: '+JSON.stringify({status:b.status,count:b.count}));
  ...
  if(changed){
    ...
    blackbox.record('IDX-078','index-stability-patched',{inGame:!!a.changed,renderAll:!!b.changed});
  }
  return{inGame:a,renderAll:b,changed};
}
try{patchInstalledIndex()}catch(e){blackbox.record('IDX-E078','index-patch-error',{message:e?.message||String(e),stack:e?.stack||''});throw e}
...
const scriptsNew="'runtime-blackbox-v01577.js','runtime-performance-v01568.js','runtime-live-autosync-v01571.js','live-strength-v01513.js'";
...
const readyNew='Boolean(window.__ARAM_BLACKBOX_V01577__) && Boolean(window.__ARAM_RUNTIME_PERFORMANCE_V01568__) && Boolean(window.__ARAM_LIVE_AUTOSYNC_RUNTIME_V01571__) && Boolean(window.__ARAM_LIVE_STRENGTH_V01513__)';
...
for(const needle of [scriptsOld,randomOld,readyOld,randomReadyOld,"const VERSION='0.15.70'"]){if(!src.includes(needle))throw new Error('v0.15.78 base main contract mismatch: '+needle.slice(0,90))}
src=src.replace(scriptsOld,scriptsNew).replace(randomOld,randomNew).replace(readyOld,readyNew).replace(randomReadyOld,randomReadyNew).replaceAll('0.15.70','0.15.78');
...
const loopNew=`let chain=require('./runtime-loader-v01577').injectRuntimeStack({mainWindow,scripts,blackbox:require('./freeze-blackbox-v01577'),patchRuntimeSource:require('./runtime-source-stability-v01578').patchRuntimeSource});`;
if(!src.includes(loopOld))throw new Error('v0.15.78 runtime loader loop contract mismatch');
...
if(!src.includes(finalOld))throw new Error('v0.15.78 runtime finalizer contract mismatch');
