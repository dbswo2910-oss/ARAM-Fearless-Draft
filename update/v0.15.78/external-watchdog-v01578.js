...
const ageOf=f=>{try{return Date.now()-fs.statSync(f).mtimeMs}catch{return Infinity}};
...
function check(){
  ...
  if(code!==lastCode||((m||r)&&Date.now()-lastWrite>8000)){
    append(code,{parentPid,mainAgeMs:Number.isFinite(mainAge)?Math.round(mainAge):null,rendererAgeMs:Number.isFinite(rendererAge)?Math.round(rendererAge):null});
