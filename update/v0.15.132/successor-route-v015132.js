...
const NEW_ROUTE_FRAGMENT="'0.15.132').replaceAll(stabilityAnchor,'runtime-source-stability-v015132')";
function patchSuccessorSource(input){
  const src=String(input||'');
  const routeHits=src.split(OLD_ROUTE_FRAGMENT).length-1;
  if(routeHits!==1)throw new Error(`v0.15.132 main successor contract mismatch: expected one v0.15.122 route, got ${routeHits}`);
