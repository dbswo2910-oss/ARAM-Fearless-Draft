...
const SENTINEL='/* ARAM_RATING_RESEARCH_UI_V015130 */';
let cached='';
function readLocal(name){
  const local=path.join(__dirname,name);
  const repo=path.join(__dirname,'..','v0.15.130',name);
...
function payloadSource(){
  ...
  if(!engine.includes('ARAMRatingResearchEngineV01'))throw new Error('v0.15.130 rating engine contract mismatch');
  if(!core.includes('ARAMRatingResearchUICoreV01'))throw new Error('v0.15.130 research UI core contract mismatch');
  if(!ui.includes('aramRatingResearchUIV01'))throw new Error('v0.15.130 research UI runtime contract mismatch');
...
function injectResearchUI(src){
  ...
  if(!src.includes('__ARAM_PLAYER_PROFILE_V01519__'))throw new Error('v0.15.130 player profile owner marker missing');
  return src+'\n'+SENTINEL+'\n'+payloadSource()+'\n';
}
