...
function statePath(){const r=root();return r?path.join(r,'promotion-v015133.json'):''}
...
function install(opts={}){const appDir=path.resolve(opts.appDir||__dirname);if(String(opts.version||VERSION)!==VERSION)throw new Error('v0.15.133 cold-start promotion version mismatch');if(!electron?.app||process.platform!=='win32')return{scheduled:false,reason:process.platform!=='win32'?'non-windows':'electron-unavailable'};const run=()=>setTimeout(()=>{promote(appDir).catch(e=>writeState({schema:1,version:VERSION,status:'failed',at:new Date().toISOString(),appDir,error:e?.message||String(e)}))},1200);if(electron.app.isReady())run();else electron.app.whenReady().then(run).catch(()=>{});return{scheduled:true,appDir,launcher:path.join(appDir,LAUNCHER_NAME)}}
