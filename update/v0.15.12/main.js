...
function createWindow(){
  ...
  mainWindow.webContents.on('dom-ready',()=>{
    try{
      const code=fs.readFileSync(path.join(__dirname,'live-strength-v01512.js'),'utf8')+'\n//# sourceURL=live-strength-v01512.js';
      mainWindow.webContents.executeJavaScript(code,false).then(()=>mainWindow.webContents.executeJavaScript('Boolean(window.__ARAM_LIVE_STRENGTH_V01512__)',false)).then(ok=>{if(!ok)console.error('[v0.15.12] live strength marker missing')}).catch(e=>console.error('[v0.15.12] live strength inject failed',e));
    }catch(e){console.error('[v0.15.12] live strength read failed',e)}
  });
