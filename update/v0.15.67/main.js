...
// v0.15.67 stability: Windows renderer/GPU compositing can hard-freeze during move/resize on some systems.
if(process.platform==='win32'&&process.env.ARAM_ENABLE_HARDWARE_ACCELERATION!=='1')app.disableHardwareAcceleration();
...
if(!app.requestSingleInstanceLock()){app.quit()}else if(isUpdateHandoff){
  ...
  app.whenReady().then(()=>{
    ...
    core.start();
    try{gradeCollector=riotGrade.createCollector(core,{app,intervalMs:5000});gradeCollector.start()}catch(e){console.error('[v0.15.67] Riot Grade collector init failed',e)}
