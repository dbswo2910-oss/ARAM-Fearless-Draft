...
for(const needle of [scriptsOld,randomOld,readyOld,randomReadyOld,"const VERSION='0.15.70'"]){if(!src.includes(needle))throw new Error('v0.15.73 base main contract mismatch: '+needle.slice(0,80))}
src=src.replace(scriptsOld,scriptsNew).replace(randomOld,randomNew).replace(readyOld,readyNew).replace(randomReadyOld,randomReadyNew).replaceAll('0.15.70','0.15.73');
