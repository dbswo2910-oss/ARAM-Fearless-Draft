...
(()=>{
  ...
  let catalog=null,pending=null,nameToId=new Map(),timer=0;
  ...
  function sync(){
    ...
    if(!catalog?.ok){loadCatalog().then(()=>sync());return}
  ...
  function start(){
    ...
    ensureStyles();loadCatalog().then(()=>sync());sync();timer=setInterval(sync,320);
    window.__ARAM_RANDOM_ITEM_ICONS_V01556__=true;
    window.aramRandomItemIconsV01556={version:V,refresh:sync,getCatalog:()=>catalog,timer,score_logic_changed:false,source:'Official Data Dragon item icons'};
  }
  start();
})();
