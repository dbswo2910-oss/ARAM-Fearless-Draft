...
(()=>{
  ...
  function partyPicks(){
    ...
    const q=Math.max(1,Math.min(5,Number(s.partySize)||1));
    const external=new Set(unique(s.external||[]));
    const picks=unique(s.party||[]);
    const local=resolveChamp(s.localChampion);
    if(local&&!picks.includes(local))picks.push(local);
    // v0.15.72 fallback: some client states expose the full team before the lobby-party
    // snapshot is complete. Derive the missing party-held champions as team minus external.
    if(picks.length<q){
      for(const n of unique(s.team||[])){
        if(external.has(n)||picks.includes(n))continue;
        picks.push(n);if(picks.length>=q)break;
      }
    }
  ...
  function start(){
    ...
    ensureStyles();sync();
    // v0.15.72: the Random Practice coordinator owns recurring refreshes. Do not attach
    // a second subtree observer, document-wide input/click listeners, or polling interval.
