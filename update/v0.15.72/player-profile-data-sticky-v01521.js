...
(()=>{
  ...
  function enhance(){
    // v0.15.35 coexistence guard: role-filtered profiles are owned by the role-specialized renderer.
    // The v0.15.21 MutationObserver must not overwrite ROLE RADAR / role-specific scout content.
    if(window.aramRoleProfileContextV01523?.selectedRole)return false;
...
`;document.head.appendChild(s)}
  ...
  function applySticky(){
    ...
    if(!dataView?.classList?.contains('active'))return false;
    dataView.querySelectorAll('.dataChampionDetailStickyV01521').forEach(x=>x.classList.remove('dataChampionDetailStickyV01521'));
    dataView.querySelectorAll('.dataChampionDetailStickyParentV01521').forEach(x=>x.classList.remove('dataChampionDetailStickyParentV01521'));
  ...
  // v0.15.72: this is a Data-tab feature. Never observe the whole app body or react to
  // Random Practice champion DOM churn. Scope observation and click work to #data only.
  const dataView=document.getElementById('data');
  if(dataView){
    const obs=new MutationObserver(()=>{if(!dataView.classList.contains('active'))return;rebind();scheduleSticky();try{if(document.getElementById('pp19ov')?.classList.contains('open'))setTimeout(enhance,0)}catch{}});
    obs.observe(dataView,{childList:true,subtree:true});
  }
  window.addEventListener('resize',()=>{if(document.getElementById('data')?.classList.contains('active'))scheduleSticky()},{passive:true});
  document.addEventListener('click',e=>{if(e.target?.closest?.('#data [class*="champ"],#data [id*="champ"],#data .dataRow,#data .dataCard'))scheduleSticky()},{capture:true});

  try{
    const oldRenderData=window.renderData;
    if(typeof oldRenderData==='function')window.renderData=function(...a){const r=oldRenderData.apply(this,a);scheduleSticky();return r};
  }catch{}
  try{
  ...
  if(typeof DATA!=='undefined'){DATA.version=V;DATA.player_profile_diversity_v01521={version:'v0.15.35 · Player Profile Diversity + Role Radar Coexistence',style_radar:'within-player contrast',archetype_count:15,balance_fallback_removed:true,sticky_champion_detail:true}}
