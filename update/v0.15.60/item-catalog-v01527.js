...
const COMMUNITY_ICON_BASE='https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/assets/items/icons2d/';
...
function getJson(url,redirects=0){
  return new Promise((resolve,reject)=>{
    ...
    const req=https.get(u,{headers:{'User-Agent':'ARAM-Fearless-Draft/0.15.60','Cache-Control':'no-cache'}},res=>{
...
function communityIconUrl(iconPath){
  const file=String(iconPath||'').split('/').pop();
  if(!file)return'';
  return `${COMMUNITY_ICON_BASE}${encodeURIComponent(file.toLowerCase())}`;
}
function communityIconMap(json){
  const out={};
  for(const row of Array.isArray(json)?json:[]){
    const id=String(Number(row?.id)||'');if(!id)continue;
    const url=communityIconUrl(row?.iconPath);if(url)out[id]=url;
  }
  return out;
}

function compact(version,json,latestIcons={}){
  ...
  for(const [id,it] of Object.entries(data)){
    ...
    const latestIcon=latestIcons[String(id)]||'';
    items[String(id)]={id:String(id),name,total,base:base||(!from.length?total:0),sell,from,into,full,map12,purchasable,depth,tags,
      iconUrl:latestIcon||ddragonIconUrl(version,id),iconSource:latestIcon?'CommunityDragon latest Riot client asset mirror':'Data Dragon fallback'};
  }
  const iconCount=Object.values(items).filter(x=>x.iconSource.startsWith('CommunityDragon')).length;
  return{ok:true,version,locale:'ko_KR',items,count:Object.keys(items).length,loadedAt:Date.now(),source:'Data Dragon realms/kr · ko_KR',recipeAware:true,
    iconSource:'CommunityDragon latest · Riot client lol-game-data mirror',iconCount,iconFallback:'Data Dragon versioned item art'};
}
...
async function fetchCatalog(){
  ...
  pending=(async()=>{
    let version='',json=null,error='',realmError='',iconError='',latestIcons={};
    try{latestIcons=communityIconMap(await getJson(COMMUNITY_ITEMS))}catch(e){iconError=e?.message||String(e)}
    ...
    cache=json?compact(version,json,latestIcons):{ok:false,version:'',locale:'',items:{},count:0,loadedAt:Date.now(),source:'fallback',recipeAware:false,
      iconSource:'CommunityDragon latest · Riot client lol-game-data mirror',iconCount:Object.keys(latestIcons).length,error:error||realmError||'Data Dragon item catalog unavailable',iconError};
...
module.exports={register,fetchCatalog,compact,communityIconMap,communityIconUrl,get cache(){return cache}};
