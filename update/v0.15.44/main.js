...
if(!app.requestSingleInstanceLock()){app.quit()}else if(isUpdateHandoff){
  ...
  app.whenReady().then(()=>{
    ...
    core.start();
    try{gradeCollector=riotGrade.createCollector(core,{app,intervalMs:5000});gradeCollector.start()}catch(e){console.error('[v0.15.44] Riot Grade collector init failed',e)}
