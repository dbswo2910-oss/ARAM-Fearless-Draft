  ...
  mainWindow.webContents.on('dom-ready',()=>{
    try{
      ...
      mainWindow.webContents.executeJavaScript(liveCode,false)
        ...
        .then(()=>mainWindow.webContents.executeJavaScript('Boolean(window.__ARAM_LIVE_STRENGTH_V01513__) && Boolean(window.__ARAM_ROLE_GRADE_V01513__) && Boolean(window.__ARAM_MATCH_LAB_FAVORITES_V01514__)',false))
        .then(ok=>{if(!ok)console.error('[v0.15.14] runtime patch marker missing')})
        .catch(e=>console.error('[v0.15.14] runtime patch inject failed',e));
    }catch(e){console.error('[v0.15.14] runtime patch read failed',e)}
  });
...
if(!app.requestSingleInstanceLock()){app.quit()}else if(isUpdateHandoff){
  ...
  app.on('second-instance',(_event,argv)=>{
    if(Array.isArray(argv)&&argv.includes('--aram-update-handoff')){
      quitting=true;try{core.stop()}catch{};app.quit();return;
    }
    showWindow();
  });
  app.whenReady().then(()=>{
