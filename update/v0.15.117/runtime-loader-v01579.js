...
function writeDiagnostic(payload){try{const dir=diagnosticDir();fs.mkdirSync(dir,{recursive:true});const dst=path.join(dir,'runtime-readiness-v015117.json'),tmp=dst+'.tmp-'+process.pid;fs.writeFileSync(tmp,JSON.stringify(payload,null,2),'utf8');try{fs.renameSync(tmp,dst)}catch{try{fs.rmSync(dst,{force:true})}catch{}fs.renameSync(tmp,dst)}}catch{}}
function markFailure(detail,blackbox){try{require('./update-safety-v01579').markSafetyFailure({code:'SAFE-RT117',detail})}catch(e){blackbox?.record?.('RTI-E117','runtime-readiness-mark-failed',{message:e?.message||String(e)})}}
async function injectRuntimeStack(opts={}){
  ...
  if(wc)try{await wc.executeJavaScript('window.aramSafetyNetV01579?.finalize?.(); true',false);blackbox?.record?.('SAFE-FIN','safety-finalize-end',{wrapped:true,policy:'0.15.117'})}catch(e){blackbox?.record?.('SAFE-FIN','safety-finalize-error',{message:e?.message||String(e)})}
  ...
  const payload={at:new Date().toISOString(),version:'0.15.117',ok,missing:Array.isArray(readiness?.missing)?readiness.missing:[],failedCritical,required:readiness?.required||{},error:readiness?.error||''};
  writeDiagnostic(payload);
  if(!ok){markFailure(payload,blackbox);blackbox?.record?.('RTI-R117','critical-runtime-readiness-failed',payload)}
  else blackbox?.record?.('RTI-OK117','critical-runtime-readiness-ok',{version:'0.15.117'});
  return result;
}
module.exports={injectRuntimeStack,_test:{CRITICAL_SCRIPT_FILES,READINESS_EXPR},score_logic_changed:false,random_scoring_changed:false,critical_runtime_readiness_gate:true,state_integrity_readiness_gate:true,policy_version:'0.15.117'};
