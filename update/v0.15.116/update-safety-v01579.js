...
function failureForBoot(root,x){const failure=readJson(failurePath(root));return failure&&Number(failure.at||0)>=Number(x?.bootStartedAt||0)?failure:null}
...
function installBootGuard(opts={}){
  ...
  if(x&&x.state==='applied'&&String(x.toVersion)===version){
    ...
    if(abnormal||hang||markedFailure){
      ...
      let healthy=0,probationBlocked=false;
      const timer=setInterval(()=>{
        const curFailure=failureForBoot(root,x);
        if(curFailure){
          healthy=0;
          if(!probationBlocked){probationBlocked=true;append(root,'PROBATION_BLOCKED',{version,code:String(curFailure.code||'SAFE-FAIL')});if(typeof opts.record==='function')opts.record('SAFE-HOLD1','probation-blocked-by-runtime-failure',{version,code:String(curFailure.code||'SAFE-FAIL')})}
          return;
        }
        ...
        if(healthy>=12){
          ...
          if(cur&&String(cur.toVersion)===version&&!failureForBoot(root,cur)){
...
module.exports={prepareUpdateTransaction,markUpdateApplied,abortUpdateTransaction,installBootGuard,markSafetyFailure,_test:{restoreSnapshot,readJson,writeJson,rootOf,parseRecentHang,failureForBoot},score_logic_changed:false,runtime_failure_probation_gate:true,policy_version:'0.15.116'};
