...
const READINESS_EXPR=`(()=>{const required={
  ...
  dataSubnav:Boolean(window.__ARAM_DATA_SUBNAV_RESTORE_V015120__),
...
async function injectRuntimeStack(opts={}){
  ...
  let readiness={ok:false,missing:['renderer-unavailable'],required:{},lifecycle:null,autosync:null,dataOwner:null};
  if(wc)try{readiness=await wc.executeJavaScript(READINESS_EXPR,false)}catch(e){readiness={ok:false,missing:['readiness-eval-error'],required:{},lifecycle:null,autosync:null,dataOwner:null,error:e?.message||String(e)}}
  const ok=!!readiness?.ok&&failedCritical.length===0;
  const payload={at:new Date().toISOString(),version:'0.15.120',ok,missing:Array.isArray(readiness?.missing)?readiness.missing:[],failedCritical,required:readiness?.required||{},lifecycle:readiness?.lifecycle||null,autosync:readiness?.autosync||null,dataOwner:readiness?.dataOwner||null,error:readiness?.error||''};
  writeDiagnostic(payload);
  if(!ok){markFailure(payload,blackbox);blackbox?.record?.('RTI-R120','data-subnav-readiness-failed',payload)}
  else blackbox?.record?.('RTI-OK120','data-subnav-readiness-ok',{version:'0.15.120',dataOwner:payload.dataOwner});
  return result;
}
module.exports={injectRuntimeStack,_test:{CRITICAL_SCRIPT_FILES,READINESS_EXPR},score_logic_changed:false,random_scoring_changed:false,critical_runtime_readiness_gate:true,state_integrity_readiness_gate:true,resource_lifecycle_readiness_gate:true,autosync_concurrency_readiness_gate:true,data_subnav_readiness_gate:true,policy_version:'0.15.120'};
